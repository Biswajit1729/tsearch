...
let magnetSearch = async function(q,l,p) {
    // Search '1080' in 'Movies' category and limit to 20 results
    try {
        const torrents = await TorrentSearchApi.search(p,q, "All", l).catch(error => {throw error}); 
       // console.log(torrents);
        const m = await TorrentSearchApi.getMagnet(torrents[l-1]).catch(error => {throw error}); 
        return m;
    } catch (error) {
        return error;
    }
   ...
   //magnet 
app.get('/magnetLink/:l', async function(req, res){
    res.header("Access-Control-Allow-Origin", "*");
    res.header("Access-Control-Allow-Methods", "GET,HEAD,OPTIONS,POST,PUT");
    res.header("Access-Control-Allow-Headers", "Origin, X-Requested-With, Content-Type,   Accept, x-client-key, x-client-token, x-client-secret, Authorization");
    ...
    if (req.query.p === undefined) {
        ...
        var q = req.query.q;
        var l =req.params.l
        var m = await magnetSearch(q,l,p).catch(error => {throw error}); 
